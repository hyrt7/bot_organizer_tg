...
Важно помнить, что порции должны быть не слишком большими,'''\
''' а калорийность питания'''\
    ''' должна соответствовать уровню активности спортсмена. Также следует'''\
    ''' учитывать индивидуальные особенности организма и '''\
...
opros_activity = ReplyKeyboardMarkup(
...
async def process_start_command(message: types.Message):
    ...
    await message.answer(text=f'Привет, {message.from_user.full_name}, добро'
                              ' пожаловать в нашего Телеграмм Ботa! Пройдите'
                              ' опрос для следущей работы. Ваш пол:', reply_markup=opros_sex)
    await bot.send_sticker(user_id, sticker='CAACAgIAAxkBAAEIAbFkBHf3USGmNp'
                                            'L6XU28yDBbKGBuiAACvQ'
                                            'wAAgbX6UmiQ5zoyiNTgS4E',)
...
async def process_activity(message: types.Message, state: FSMContext):
    ...
    await message.answer('Спасибо за прохождение опроса! Подгототавливаю список упражнений.\
    Пожалуйста, установите цель или цели используя /goal', reply_markup=menu)
...
async def process_exercises(message: types.Message):
    ...
    result = cursor.execute(
        f'''SELECT activity_level FROM log_entries_1 WHERE user_id = {message.from_user.id}''').fetchall()
    if result[0][0][0] == '1':
...
async def process_nutrition(message: types.Message):
    ...
    menu.add()
    await message.answer('''1. Питайтесь разнообразно и сбалансированно,'''
                         ''' употребляйте продукты всех групп пищевых'''
                         ''' веществ, таких как белки, углеводы и жиры.

2. Ограничьте потребление продуктов, '''
                         '''содержащих большое количество животных жиров,'''
                         ''' холестерина, сахара, соли и пустых углеводов.

3. Употребляйте меньшие порции еды, но чаще - 5-6 раз в день.

4. Увеличьте потребление овощей, '''
                         '''фруктов, ягод, злаковых продуктов, бобовых и орехов.
...
7. Придерживайтесь правильного режима питания, '''
                         '''не пропускайте завтрак и ужинайте за 2-3 часа до сна.
...
async def process_goal_button(message: types.Message):
    ...
    result = cursor.execute(
        f'''SELECT entry FROM log_entries_1 WHERE user_id = {message.from_user.id}''').fetchall()
    await message.answer(f'''Ваша цель - {result[0][0]}
    Я помогу вам с мотивацией:\n\" Я скажу то, что для тебя не новость: мир\
     не такой уж солнечный и приветливый. Это очень опасное, жесткое место, и если только дашь слабину, он опроки
нет с такой силой тебя, что больше уже не встанешь. Ни ты, ни я, никто на \
свете, не бьёт так сильно, как жизнь! Совсем не важно, как ты ударишь,\
 а важно, какой держишь удар, как двигае
шься вперёд. Будешь идти — ИДИ! Если с испугу не свернёшь... Только так \
побеждают! Если знаешь, чего ты стоишь — иди и бери своё!\" - Рокки Бальбоа''')
...
def add_log_entry(user_id, entry, activity_level):
    ...
    cursor.execute("INSERT INTO log_entries_1 VALUES (?, ?, ?, ?)",
                   (user_id, entry, activity_level, date))
    conn.commit()


@dp.message_handler(commands=['goal'])
async def achievments(message: types.Message):
    await message.reply("Что ты выберешь в свою цель?", reply_markup=goal_kb)
...
async def remind_about_exercises():
    while True:
        ...
        result = cursor.execute(
            f'''SELECT user_id FROM log_entries_1''').fetchall()
        for i in result:
...
async def remind_about_goal():
    while True:
        ...
        for i in result:
            result = cursor.execute(
                f'''SELECT entry FROM log_entries_1 WHERE user_id = {i[0]}''').fetchall()
